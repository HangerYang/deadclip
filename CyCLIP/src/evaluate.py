...
def evaluate(epoch, model, processor, data, options):
    ...
    if(options.master):
        if(options.representation):
            similarity_score = get_similarity_score(model, data["train"], options)
            np.savez("/home/hyang/deadclip/checkmate/{}_{}".format(options.name, options.epochs), similarity_score)
            return 0
...
def get_similarity_score(model, dataloader, options):
    ...
    with torch.no_grad():
        ...
        for batch in tqdm(dataloader):
            ...
            total_similarity_score = np.concatenate((total_similarity_score, similarity_matrix), 0)
